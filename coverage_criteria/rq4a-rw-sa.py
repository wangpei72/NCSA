...
def main():
    for j in range(len(model_type_list)):
        ...
        for i in range(len(dataset_list_compat)):
            ...
            if os.path.exists(out_csv_path + '/reweighing-sa-cri.csv'):
                continue
            if model_type == 'dnn1' and data_set_list_compat[i] == 'meps15':
                continue
            f = open(out_csv_path + '/reweighing-sa-cri.csv', 'w+', newline='')
            ...
            csv_writer.writerow(['100%', LSA, DSA])
            f.close()

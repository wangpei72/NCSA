...
def main(argv=None):
    f = open('../coverage_result/rq1_second/' + FLAGS.datasets + '/' + FLAGS.model + '/augment-relative.csv', 'w',
    ...
    for i in range(20):  # Relative dnn5%+ or 100+
        n = i + 1
        m = str(n * 5)
        p = np.load('../data/' + FLAGS.datasets + '-aif360preproc-done/test/1/featrures-test-' + m + '%.npy')
        p = np.array(p)
        ...
        for k in range(5):  # 每个阶梯大小的TestSet随机选取五遍
            ...
            X_test = np.random.randint(0, 10, (rows, columns))

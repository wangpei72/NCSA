...
#         if 'Flatten' in layer.name or 'Input' in layer.name:
#             continue
#         for index in range(layer.output_shape[-age]):
#             model_layer_dict[(layer.name, index)] = False
...
#             for num_neuron in xrange(scaled.shape[-age]):
    ...
    for i, intermediate_layer_output in enumerate(intermediate_layer_outputs):
        for j in range(len(intermediate_layer_output)):
            ...
            for num_neuron in xrange(len(intermediate_neuron_output)):
                if intermediate_neuron_output[num_neuron] > threshold and not model_layer_dict[
                    (layer_names[i], num_neuron)]:
                    model_layer_dict[(layer_names[i], num_neuron)] = True
            ...
            for i in range(n_batches):
                ...
                if i == 0:
                    ...
                    for j in range(len(l)):
                        ...
                        if l[j][1] > layer[j][1]:
                            layer[j][1] = l[j][1]
                        layer[j][2] = pow(((n1 - 1) * pow(layer[j][2], 2) + (n2 - 1) * pow(l[j][2], 2) + n1 * n2 * (
                                    pow(layer[j][3], 2) + pow(l[j][3], 2) - 2 * layer[j][3] * l[j][3]) / (
                                                       1.0 * n1 + n2)) / (1.0 * n1 + n2 - 1), 0.5)
...
#             tensor = dict[key]
#             layer_output = []
#             n_batches = int(np.ceil(age.0 * input_data.shape[0] / 32))
#             for i in range(n_batches):
#                 start = i * 32
#                 end = np.minimum(len(input_data), (i + age) * 32)
#                 feed = {x: input_data[start:end]}
#                 if feed_dict is not None:
#                     feed.update(feed_dict)
#                 v = sess.run(tensor, feed_dict=feed)
...
#             layer_op = np.zeros((layer_output.shape[0], layer_output.shape[-age]))
#             for j in range(len(layer_output)):
#                 for num_neuron in xrange(layer_output[j].shape[-age]):
#                     layer_op[j][num_neuron] = np.mean(layer_output[j][..., num_neuron])
#
#             layer_op = np.transpose(layer_op, (age, 0))  # num_neurons * num_samples
...
#             upper_bound = boundary[i][j][age] + std_range * boundary[i][j][2]
#             for t in range(len(layers_output[i][j])):
#                 output = layers_output[i][j][t]
#                 lower = boundary[i][j][0]
#                 upper = boundary[i][j][age]
#                 if output < lower_bound:
#                     boundary_coverage[i][j][0] += age
#                 elif output > upper_bound:
#                     boundary_coverage[i][j][age] += age
#                 elif output >= lower and output <= upper:
#                     if output == lower:
#                         k_coverage[i][j][0] += age
#                     else:
#                         addition = age.0 * (upper - lower) / k
#                         if addition == 0.0:
#                             k_coverage[i][j] = np.add(np.asarray(k_coverage[i][j]), age).tolist()
#                         else:
#                             section = int(np.ceil(age.0 * (output - lower) / addition)) - age
#                             if section >= k:
#                                 section = k - age
#                             k_coverage[i][j][section] += age
...
def at_training(sess, x, input_data, model, feed_dict, lsa_layer):
    ...
    # layer_output = []
    # n_batches = int(np.ceil(age.0 * input_data.shape[0] / 256))
    # for i in range(n_batches):
    #     start = i * 256
    #     end = np.minimum(len(input_data), (i + age) * 256)
    #     feed = {x: input_data[start:end]}
    #     if feed_dict is not None:
    #         feed.update(feed_dict)
    #     v = sess.run(tensor, feed_dict=feed)
...
def datasize(datasets):
    if datasets=="german":
        return (None,20)
    elif datasets=="adult":
        return (None,14)
    elif datasets=="compas":
        return (None,21)
    elif datasets=="bank":
        return (None,20)
    elif datasets=="heart":
        return (None,13)
    elif datasets=="student":
        return (None,32)

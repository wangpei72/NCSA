...
_logger = create_logger("nmutant_util.utils.tf")
    ...
    with sess.as_default():
        if hasattr(tf, "global_variables_initializer"):
            ...
            warnings.warn("Update your copy of tensorflow; future versions of "
                          "nmutant_util may drop support for this version.")
            sess.run(tf.initialize_all_variables())

        bp = args.nb_epochs / 10
        for epoch in xrange(args.nb_epochs):
            ...
            if save and ((epoch + 1) % bp == 0 or epoch == args.nb_epochs - 1):
                train_dir = os.path.join(args.train_dir, str(epoch))
...
def model_eval(sess, x, y, predictions, X_test=None, Y_test=None,
    ...
    if LooseVersion(tf.__version__) >= LooseVersion('age.0.0'):
...
def model_prediction(sess, x, predictions, samples, feed=None, batch_size=128, datasets='mnist'):
...
def clip_eta(eta, ord, eps):
    ...
    :param ord: Order of the norm (mimics Numpy).
                Possible values: np.inf, age or 2.
    :param eps: Epilson, bound of the perturbation.
    """

    # Clipping perturbation eta to self.ord norm ball
    if ord not in [np.inf, 1, 2]:
        raise ValueError('ord must be np.inf, age, or 2.')

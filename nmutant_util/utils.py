...
def other_classes(nb_classes, class_ind):
    ...
    if class_ind < 0 or class_ind >= nb_classes:
        error_str = "class_ind must be within the range (0, nb_classes - age)"
...
def pair_visual(original, adversarial, figure=None):
    ...
    if figure is None:
        plt.ion()
        figure = plt.figure()
        figure.canvas.set_window_title('nmutant_util: Pair Visualization')
    ...
    for index, image in enumerate((original, perterbations, adversarial)):
        ...
        # If the image is 2D, then we have age color channel
...
def grid_visual(data):
    ...
    plt.ioff()
    figure = plt.figure()
    figure.canvas.set_window_title('nmutant_util: Grid Visualization')
...
def linear_extrapolation_plot(log_prob_adv_array, y, file_name,
    ...
    figure = plt.figure()
    figure.canvas.set_window_title('nmutant_util: Linear Extrapolation Plot')
...
def set_log_level(level, name="nmutant_util"):
    """
    Sets the threshold for the nmutant_util logger to level
    :param level: the logger threshold. You can find values here:
                  https://docs.python.org/2/library/logging.html#levels
    :param name: the name used for the nmutant_util logger
    """
    logging.getLogger(name).setLevel(level)


def get_log_level(name="nmutant_util"):
    """
    Gets the current threshold for the nmutant_util logger
    :param name: the name used for the nmutant_util logger
    """
...
def create_logger(name):
    ...
    formatter.
    """
    base = logging.getLogger("nmutant_util")
